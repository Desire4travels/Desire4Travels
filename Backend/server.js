...
const ownerAuth = (req, res, next) => {
  ...
  return res
    .status(403)
    .json({ error: "Only the owner can delete enquiries." });
};
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json({ message: "Destination added successfully", id: docRef.id });
    } catch (error) {
...
app.get("/api/destinations", async (req, res) => {
  try {
    ...
    const paginated = allDestinations.slice(
      startIndex,
...
app.get("/api/packages", async (req, res) => {
  try {
    ...
    const packagesRaw = snapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
    ...
    const destDocs = await Promise.all(
      destIdArray.map((id) => db.collection("destinations").doc(id).get())
    );
...
app.get("/api/packages/:id", async (req, res) => {
  try {
    ...
    const cleanDescription = packageData.description?.replace(
      /(<([^>]+)>)/gi,
      ""
    ); // remove HTML tags
...
app.put("/callback-destination/:id", async (req, res) => {
  try {
    ...
    if (typeof called !== "boolean") {
      return res
        .status(400)
        .json({ error: "'called' must be a boolean value." });
    }
  ...
  try {
    ...
    if (!name || !phoneNo || !email || !message) {
      return res
        .status(400)
        .json({
          error: "All fields (name, phoneNo, email, message) are required.",
        });
    }
    ...
    res
      .status(200)
      .json({ message: "Contact message sent successfully", id: docRef.id });
  } catch (error) {
    console.error("POST /contact-us ERROR:", error);
    res.status(500).json({ error: "Internal server error" });
  }
...
app.post("/activity-callback", async (req, res) => {
  try {
    ...
    if (!number || !activityName) {
      return res
        .status(400)
        .json({ error: "Number and activityName are required" });
    }
    ...
    res
      .status(201)
      .json({ id: newDoc.id, number, activityName, called: false });
  } catch (err) {
...
app.post("/api/upcoming-trips", async (req, res) => {
  try {
    ...
    if (!tripName || !travelDate) {
      return res
        .status(400)
        .json({ error: "Trip name and travel date are required" });
    }
...
app.post("/services", async (req, res) => {
  try {
    const { city, serviceType, providerName, contactInfo, category, notes } =
      req.body;
...
app.get("/api/last-visit", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        lastVisited: data?.[section]
          ? data[section].toDate().toISOString()
          : null,
      });
  } catch (error) {
...
app.get("/service-providers", async (_req, res) => {
  try {
    ...
    for (const [type, col] of Object.entries(colMap)) {
      ...
      snap.forEach((doc) => results.push({ id: doc.id, type, ...doc.data() }));
    }
    res.json(results);
  } catch (err) {
...
app.put("/service-providers/:type/:id", async (req, res) => {
  ...
  try {
    ...
    await db.collection(colMap[type]).doc(id).set(req.body, { merge: true }); // <‑‑ merge ensures only provided fields change
...
app.put('/trip-requests/:id', async (req, res) => {
  const { id } = req.params;
  const { responses, contactNumber, ...restOfData } = req.body; // <-- add contactNumber
  ...
  if (contactNumber) {
    updatedData.contactNumber = contactNumber; // <-- add this line
  }
